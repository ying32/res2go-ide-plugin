...
procedure CtlWriteln(AUrgency: TMessageLineUrgency; const AFmt: string); inline; overload;
...
var
  uGoPkgName: string = 'main'; // 使用全局的，懒得添加额外的改变了
...
procedure CtlWriteln(AUrgency: TMessageLineUrgency; const AFmt: string; AArgs: array of const); overload;
begin
 // if AUrgency = mluError then
 //   AUrgency := mluWarning // 不能用mluError，不然IDE会死
 //else if AUrgency = mluWarning then
 //   AUrgency := mluHint;
...
{$ENDIF}
  try
    ...
    begin
      ...
      begin
        if IsZhLang then
          CtlWriteln(mluError, '“%s:%s”不被支持。', [C^.Name, C^.ClassName])
        else
          CtlWriteln(mluError, '"%s:%s" is not supported.', [C^.Name, C^.ClassName]);
        //Exit;
      end;
      ...
      begin
        if IsZhLang then
          CtlWriteln(mluWarning, '“%s:%s”必须首字母大写才能被导出。',
            [C^.Name, C^.ClassName])
        else
          CtlWriteln(mluWarning, '"%s:%s" must be capitalized first to be exported.',
            [C^.Name, C^.ClassName]);
        Continue;
      end;
  ...
  try
    ...
    try
      try
        ...
        try
          ...
          try
            ...
            try
              ...
              // 是否使用原始的dfm/lfm文件名。
              //LOrigfn := False; //FindCmdLineSwitch('origfn');
              // 提取单元名称
              LtempFileName := '';
              //if LOrigfn then
              //begin
        ...
        on E: Exception do
        begin
         CtlWriteln(mluError, '%s', [E.message]);
        end;
      end;
    finally
      LOutput.Free;
...
begin
  ...
  try
    ...
    LPkg := '';
    if (uGoPkgName <> '') and (uGoPkgName <> 'main') then
      LPkg := uGoPkgName + '.';
